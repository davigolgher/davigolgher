...
/** Expense row: name + date on the left, amount on the right, hairline divider. */
    ...
    >
      <span className="min-w-0">
        <span className="flex items-center gap-2">
          <span className="truncate text-[15px] font-semibold text-chalk">{transaction.description}</span>
        ...
        <span className="mt-0.5 block text-[13px] text-chalk-mute">{formatDateMedium(transaction.date)}</span>
      </span>
      <span className="shrink-0 text-[15px] font-semibold tabular-nums text-chalk">{money.format(transaction.amount)}</span>
    </button>
  );
}
